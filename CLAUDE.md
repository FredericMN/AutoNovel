...
**核心功能**：
1. **双版本伏笔管理**
   - **详细版** (`plot_arcs.txt`)：完整记录所有伏笔、冲突、谜团，按ABC级分类，供人工查看和一致性校审
   - **精简版** (融入 `global_summary.txt`)：自动提炼核心伏笔（A级5条+B级3条，≤200字），供LLM生成时感知

2. **ABC分级系统**（记录时即分级）
   - **[A级-主线]**：直接影响故事核心走向（世界观谜团、主角核心秘密、最终boss）
   - **[B级-支线]**：影响重要配角或次要情节线（配角背景、势力冲突）
   - **[C级-细节]**：仅影响局部细节（物品来源、小道具、一次性角色）

3. **定稿流程增强** (`novel_generator/finalization.py`)
   ```
   ...
   [2.8/3] 🆕 提炼伏笔到摘要（精简版，筛选ABC级） → 追加到 global_summary.txt
   [3/3] 插入向量库
   ```

4. **智能压缩机制**（每10章自动触发）
   - **触发条件**：章节号是10的倍数 且（未解决>50条 OR 已解决>20条）
   - **A级**：强制≤30条（相关伏笔合并）
   - **B级**：≤10条（保留最近20章）
   - **C级**：≤3条（保留最近3章）
   - **已解决**：≤10条
...
5. **分级传递策略**
   - **章节传递**：A级（5条）+ B级（3条），分隔符：`━━━ 未解决伏笔（至第X章） ━━━`
   - **卷传递**：A级（全部）+ B级（全部），分隔符：`━━━ 第X卷未解决伏笔 ━━━`
   - C级细节不跨章节传递
...
**技术实现**：
- **提示词**：
  - `plot_arcs_update_prompt`：记录伏笔（按ABC级分类）
  - `plot_arcs_compress_auto_prompt`：智能压缩（周期性触发）
  - `plot_arcs_distill_prompt`：提炼核心伏笔
  - `plot_arcs_compress_prompt`：二次压缩（超限时）
  - ⚠️ `plot_arcs_classify_prompt`：已废弃（记录时直接分级）
- **配置**：`prompts_config.json` 新增四个模块，用户可自定义提示词
- **文件**：
  - `core/prompting/prompt_definitions.py` - 提示词定义
  - `novel_generator/finalization.py` - 核心逻辑实现
  - `custom_prompts/plot_arcs_*.txt` - 自定义提示词模板
  - `docs/plot_arcs_workflow.md` - 完整流程文档
...
4. **开关控制**：
   - 步骤2.5（剧情要点更新）：记录未解决伏笔，按ABC级分类
   - 步骤2.6（智能自动压缩）：周期性压缩详细版（每10章，阈值：未解决>50条/已解决>20条，A级≤30条）
   - 步骤2.8（伏笔提炼）：提炼核心伏笔融入摘要
...
- 🎯 智能压缩（A级≤30条，避免详细版过度膨胀）
- 🛡️ 鲁棒匹配（宽松格式识别，容忍LLM输出变体）

**格式兼容性**：
- 未解决伏笔：支持 `[A级-...]`、`- [B级-...]`、`• [C级-...]` 等多种前导符号和空格变体
- 已解决伏笔：支持 `✓已解决`、`✅已解决`、`☑已解决` 等多种符号和冒号变体
- 计数逻辑：使用正则表达式宽松匹配，避免因格式细微差异导致计数不准

**详细文档**：见 `docs/plot_arcs_workflow.md`
