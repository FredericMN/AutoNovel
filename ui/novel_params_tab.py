...
def build_novel_params_area(self, start_row=1):
    ...
    self.params_frame = ctk.CTkScrollableFrame(
        ...
        label_text="小说参数",
        label_font=IOSFonts.get_font(12, "bold")
    )
    ...
    # 0) 创作模式选择器
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="创作模式:", tooltip_key="creation_mode", row=1, column=0, font=IOSFonts.get_font(12))
    ...
    # 1) 主题(Topic) - 灵感模式下显示
    self.topic_label_frame = create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="主题(Topic):", tooltip_key="topic", row=2, column=0, font=IOSFonts.get_font(12), sticky="ne")
    self.topic_text = ctk.CTkTextbox(self.params_frame, height=70, wrap="word", font=IOSFonts.get_font(12))
    ...
    self.user_concept_label_frame = create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="故事构思:", tooltip_key="user_concept", row=2, column=0, font=IOSFonts.get_font(12), sticky="ne")
    self.user_concept_text = ctk.CTkTextbox(self.params_frame, height=150, wrap="word", font=IOSFonts.get_font(12))
    ...
    # 2) 类型(Genre)
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="类型(Genre):", tooltip_key="genre", row=3, column=0, font=IOSFonts.get_font(12))
    genre_entry = ctk.CTkEntry(self.params_frame, textvariable=self.genre_var, font=IOSFonts.get_font(12))
    genre_entry.grid(row=3, column=1, padx=5, pady=5, sticky="ew")

    # 3) 小说结构（章节数 & 每章字数 & 分卷数量）
    row_for_chapter_and_word = 4
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="小说结构:", tooltip_key="num_chapters", row=row_for_chapter_and_word, column=0, font=IOSFonts.get_font(12))
    ...
    # 章节数
    num_chapters_label = ctk.CTkLabel(chapter_word_frame, text="章节数:", font=IOSFonts.get_font(12))
    ...
    self.num_chapters_entry = ctk.CTkEntry(num_chapters_container, textvariable=self.num_chapters_var, width=55, font=IOSFonts.get_font(12))
    self.num_chapters_entry.pack(side="left", padx=(0, 2))

    # 章节数锁定图标
    self.num_chapters_lock_label = ctk.CTkLabel(num_chapters_container, text="", font=IOSFonts.get_font(14), text_color="gray", width=20)
    self.num_chapters_lock_label.pack(side="left")

    # 每章字数
    word_number_label = ctk.CTkLabel(chapter_word_frame, text="每章字数:", font=IOSFonts.get_font(12))
    word_number_label.grid(row=0, column=2, padx=(10, 3), pady=3, sticky="e")
    word_number_entry = ctk.CTkEntry(chapter_word_frame, textvariable=self.word_number_var, width=55, font=IOSFonts.get_font(12))
    word_number_entry.grid(row=0, column=3, padx=3, pady=3, sticky="w")

    # 分卷数量
    num_volumes_label = ctk.CTkLabel(chapter_word_frame, text="分卷数:", font=IOSFonts.get_font(12))
    ...
    self.num_volumes_entry = ctk.CTkEntry(num_volumes_container, textvariable=self.num_volumes_var, width=55, font=IOSFonts.get_font(12))
    self.num_volumes_entry.pack(side="left", padx=(0, 2))

    # 分卷数锁定图标
    self.num_volumes_lock_label = ctk.CTkLabel(num_volumes_container, text="", font=IOSFonts.get_font(14), text_color="gray", width=20)
    ...
    # 4) 保存路径
    row_fp = 5
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="保存路径:", tooltip_key="filepath", row=row_fp, column=0, font=IOSFonts.get_font(12))
    ...
    filepath_entry = ctk.CTkEntry(self.filepath_frame, textvariable=self.filepath_var, font=IOSFonts.get_font(12))
    filepath_entry.grid(row=0, column=0, padx=3, pady=3, sticky="ew")
    browse_btn = ctk.CTkButton(self.filepath_frame, text="浏览...", command=self.browse_folder, width=60, font=IOSFonts.get_font(12))
    browse_btn.grid(row=0, column=1, padx=3, pady=3, sticky="e")

    # 5) 章节号
    row_chap_num = 6
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="章节号:", tooltip_key="chapter_num", row=row_chap_num, column=0, font=IOSFonts.get_font(12))
    chapter_num_entry = ctk.CTkEntry(self.params_frame, textvariable=self.chapter_num_var, width=80, font=IOSFonts.get_font(12))
    chapter_num_entry.grid(row=row_chap_num, column=1, padx=5, pady=5, sticky="w")

    # 6) 内容指导
    row_user_guide = 7
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="内容指导:", tooltip_key="user_guidance", row=row_user_guide, column=0, font=IOSFonts.get_font(12), sticky="ne")
    self.user_guide_text = ctk.CTkTextbox(self.params_frame, height=70, wrap="word", font=IOSFonts.get_font(12))
    ...
    row_idx = 8
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="核心人物:", tooltip_key="characters_involved", row=row_idx, column=0, font=IOSFonts.get_font(12))
    ...
    # 三行文本输入框
    self.char_inv_text = ctk.CTkTextbox(char_inv_frame, height=60, wrap="word", font=IOSFonts.get_font(12))
    ...
    row_idx += 1
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="关键道具:", tooltip_key="key_items", row=row_idx, column=0, font=IOSFonts.get_font(12))
    key_items_entry = ctk.CTkEntry(self.params_frame, textvariable=self.key_items_var, font=IOSFonts.get_font(12))
    key_items_entry.grid(row=row_idx, column=1, padx=5, pady=5, sticky="ew")
    row_idx += 1
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="空间坐标:", tooltip_key="scene_location", row=row_idx, column=0, font=IOSFonts.get_font(12))
    scene_loc_entry = ctk.CTkEntry(self.params_frame, textvariable=self.scene_location_var, font=IOSFonts.get_font(12))
    scene_loc_entry.grid(row=row_idx, column=1, padx=5, pady=5, sticky="ew")
    row_idx += 1
    create_label_with_help_for_novel_params(self, parent=self.params_frame, label_text="时间压力:", tooltip_key="time_constraint", row=row_idx, column=0, font=IOSFonts.get_font(12))
    time_const_entry = ctk.CTkEntry(self.params_frame, textvariable=self.time_constraint_var, font=IOSFonts.get_font(12))
...
def create_label_with_help_for_novel_params(self, parent, label_text, tooltip_key, row, column, font=None, sticky="e", padx=5, pady=5):
    ...
    btn = ctk.CTkButton(frame, text="?", width=22, height=22, font=IOSFonts.get_font(10),
